...
def search_connection(b1, outlet, b2, inlet):
    ...
    for c in memory_connections:
        if ((b1 == c.box_orig) & (outlet == c.outlet) & (b2 == c.box_dest)
                & (inlet == c.inlet)):
            return i
        i += 1
...
class Connection:
    ...
    def create(self):
        ...
        if (b1 > -1) & (b2 > -1):
            ...
            command = Connection.canvas + "connect " + \
                str(b1) + " " + str(self.outlet) + " " + \
                str(b2) + " " + str(self.inlet) + " ; "
    ...
    def delete(self):
        ...
        if (b1 > -1) & (b2 > -1):
            ...
            command = Connection.canvas + "disconnect " + \
                str(b1) + " " + str(self.outlet) + " " + \
                str(b2) + " " + str(self.inlet) + " ; "
            Connection.snd.send_pd(command)
            ...
            if t1 != t2:
                i = search_connection(
                    self.box_orig, self.outlet, self.box_dest, self.inlet)
