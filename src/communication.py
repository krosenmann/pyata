...
from basic_classes.box import Box
from basic_classes.number import Number
from basic_classes.symbol import Symbol
from basic_classes.connection import Connection
...
class RemotePd(Thread):
    ...
    def run(self):
        if self.nogui:
            temp = "cd %s && pd -nogui %s/server.pd" % (
                self.pd_dir, self.server_dir)
        else:
...
class Communication():
    ...
    def init_pd(self):
        ...
        try:
            ...
            return True
        # TODO: Make this exception more concrete
        except Exception as err:
            print("Error connecting to %s:%d: %s" %
                  (self.host, self.snd_port, err))
            return False
